...
async function fetchSymbolInfo(symbol) {
    ...
    try {
        ...
        // بازگرداندن داده‌ها در قالب مورد نیاز
...
app.post("/chart", async (req, res) => {
    const { symbol, timeframe, lan, theme } = req.body;
...
app.get("/arz", async (req, res) => {
    const symbol = req.query.symbol?.toUpperCase();
    const lan = req.query.lan?.toLowerCase() || "en";

    if (!symbol) {
        return res.status(400).json({
            error: lan === "fa" ? "❌ لطفاً نماد ارز را ارسال کنید (symbol)." : "❌ Please provide a `symbol`."
        });
    }
    ...
    if (!info) {
        return res.status(500).json({
            error: lan === "fa" ? "❌ خطا در دریافت اطلاعات ارز از صرافی." : "❌ Error fetching symbol info from exchange."
        });
    }
